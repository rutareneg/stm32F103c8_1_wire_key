...
//*******************************************************************************************************************************************//
    ...
    {  IWDG_ReloadCounter();//перезапуск вачдога
    	   ...
    	   while(keys!=2)
    	   {
    	         		...
    	         		SRC = chek_crc(code,7);
    	         		if(code[7]!=SRC) {lcd_set_strs(2,0,16,"  ERR_CRC  ",0); BIP(150,300);delay_s(1); code[0]=0;break;}

    	         		for(u8 i=6;i<255;i--){

    	         			_sprtf16(data,code[i]); buf[(6-i)*2]= data[0];
    	         						buf[(6-i)*2+1]= data[1];
    	         						buf[(6-i)*2+2]= 0;}
    	      	   	...
    	      	   	switch ( code[0] ) {
    	      	   	case 0x00:     lcd_set_strs(0,0,8,"КЛЮЧ НЕ НАЙДЕН",0); lcd_set_strs(5,0,8,"              ",0); delay_ms(400); break; //собщение
    	      	   	case 0x01:     lcd_set_strs(0,0,8,"1990,2401,2411",0); lcd_set_strs(5,0,8,"   ЗАПИСАТЬ   ",0); delay_ms(400); break; //собщение
    	      	   	case 0x02:     lcd_set_strs(0,0,8,"     DS1991   ",0); lcd_set_strs(5,0,8,"НЕТ  ПРОТАКОЛА",0); delay_ms(400); break;//собщение
    	      	   	case 0x04:	   lcd_set_strs(0,0,8,"DS1994 DS2404 ",0); lcd_set_strs(5,0,8,"НЕТ  ПРОТАКОЛА",0); delay_ms(400);break;
    	      	   	case 0x06:	   lcd_set_strs(0,0,8," DS1993L 512bt",0); lcd_set_strs(5,0,8,"НЕТ  ПРОТАКОЛА",0); delay_ms(400); break;
    	      	   	case 0x08:	   lcd_set_strs(0,0,8," DS1992L 128bt",0); lcd_set_strs(5,0,8,"НЕТ  ПРОТАКОЛА",0); delay_ms(400); break;
    	      	   	case 0x0C:	   lcd_set_strs(0,0,8,"DS1996L 8192bt",0); lcd_set_strs(5,0,8,"НЕТ  ПРОТАКОЛА",0); delay_ms(400);break;
    	      	   	default:  	   lcd_set_rect(0,0,84,1); lcd_set_strs(0,0,8,"  НЕТ В БАЗЕ  ",0);	break;
